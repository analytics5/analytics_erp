...
@app.callback(dash.dependencies.Output('interface', 'labelStyle'),
              # здесь на вход принимается значение чеклиста 'colums', если значение выбрано, то отрисовывается новый блок со списком, как в дереве
...
@app.callback(dash.dependencies.Output('datatable', 'rows'),
              # здесь на вход принимается значение значение выпадающих списков и на основе этих значений формируется dataframe, посещаемый в таблицу
...
def update_datatable(Year, Country, Agency, City, Property_name, Class, SQM, Business_Sector, Type_of_Deal,
    ...
    #print(all_deals_query_df[(all_deals_query_df['Agency'].isin(['Colliers']))])
    if len(list_of_values_copy) == 0 or (Year is not None and Year[0] == 'All years'):
        ...
        for i in range(len(list_of_values_copy)):
            ...
            if command_line == "":
                print("i=", i)
                print("ind=", ind)
                print("command_line_before_up_1=", command_line)
                print("list_of_values_copy[i]=", list_of_values_copy[i])
                print("list_of_values_copy[i][0]=", list_of_values_copy[i][0])
                command_line += "(all_deals_query_df['{0}'].isin({1}))".format(ind, list_of_values_copy[i])
                print("command_line_after_up_1=", command_line)

            else:
                print("i_2", i)
                print("ind_2", ind)
                print("command_line_before_up_2=", command_line)
                print("list_of_values_copy[i]=", list_of_values_copy[i])
                command_line += "& (all_deals_query_df[{0}].isin(list_of_values_copy[{1}]))".format(ind, i)
                print("command_line_after_up_2=", command_line)

        data = all_deals_query_df[exec(command_line)]
...
def update_graph_tab(Year, Country, Agency, City, Property_name, Class, SQM, Business_Sector, Type_of_Deal,
    ...
    if len(list_of_values_copy) != 0:
        ...
        for i in range(len(list_of_values_copy)):
            ...
            df_plot = all_deals_query_df[exec(command_line)]
...
def update_datatable_susp(Year, Country, Agency, City, Property_name, Class, SQM, Type_of_Consultancy, Quarter,
    ...
    if len(list_of_values_copy) == 0:
        ...
        for i in range(len(list_of_values_copy)):
            ...
            data = all_deals_query_df[command_line]

            print(list_of_values_copy[0], 'условие', len(list_of_values_copy))
            return data.to_dict('records')
