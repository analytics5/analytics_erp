...
def update_pie_graph(Year, Country, Agency, City, Property_Name, Class, SQM, Business_Sector, Type_of_Deal,
    ...
    colors_pie = [color.colliers_grey_40, color.colliers_extra_light_blue, color.colliers_dark_blue, color.colliers_yellow,
...
def update_graph_percent(Year, Country, Agency, City, Property_Name, Class, SQM, Business_Sector, Type_of_Deal,
    ...
    trace1 = go.Bar(x=pv1.index, y=pv1[("SQM", 'Colliers')] * 100 / pv2["SQM"].sum(), name='Colliers', marker=dict(
        color=color.colliers_dark_blue), width=0.25, text='%', textposition='auto')
        ...
        html.Div(
            [
                html.Div(
                    [
                        dcc.Checklist(  # чеклист с одним значением для имитации дерева параметров
                            id='tree-checklist_susp',  # при выборе параметра, открывается список для отображения столбцов
                            options=[
                                {'label': 'Columns', 'value': 'Show'}
                            ],
                            values=[],
...
@app.callback(dash.dependencies.Output('interface_susp', 'labelStyle'),  # на вход принимается значение чеклиста 'colums'
              [dash.dependencies.Input('tree-checklist_susp', 'values')  # если значение выбрано, то отрисовывается новый блок со списком, как в дереве
               ])
def show_tree(val):
    if 'Show' in val:
        children = {'display': 'block',
...
def update_datatable_susp(Year, Country, Agency, City, Property_Name, Class, SQM, Business_Sector, Type_of_Deal,
    ...
    if len(list_of_values_copy) == 0:
        return suspecious_deals_df_no[col].to_dict('records')
    if len(list_of_values_copy) != 0:
        for i in range(len(list_of_values_copy)):
            ind = get_key(cond_1, [list_of_values_copy[i]])
            if i == 0:
                data = suspecious_deals_df_no[(suspecious_deals_df_no[ind].isin(list_of_values_copy[i]))]
            else:
                data = suspecious_deals_df_no[(suspecious_deals_df_no[ind].isin(list_of_values_copy[i]))]
        return data[col].to_dict('records')
...
@app.callback(
    dash.dependencies.Output('download-example-button', 'href'), # формирования файла-шаблона с первой строкой - названием столбцов таблицы
...
def save_contents(contents_save, filename_save):  # данные, загруженные в скрипт сохраняются в скрытом элементе страницы
    content_type, content_string = contents_save.split(',')  # в json массиве, это необходимо для передачи данных между callback`ами
    decoded = base64.b64decode(content_string + "==")  # это решение - единственный способ передать данные без использования глобальных переменных
    try:
